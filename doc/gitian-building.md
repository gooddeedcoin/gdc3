...
*Setup instructions for a gitian build of Gdc using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gdc
...
Clone the git repositories for gdc and gitian and then checkout the gdc version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/MegaPixel-UA/gdc.git
cd gdc
...
Follow the instructions in [doc/release-process.md](release-process.md) in the gdc repository
...
To build Gdc (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the gdc repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gdc/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/MegaPixel-UA/gdc
...
For example:
```bash
URL=https://github.com/crowning-/gdc.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gdc=${COMMIT} --url gdc=${URL} ../gdc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gdc=${COMMIT} --url gdc=${URL} ../gdc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gdc=${COMMIT} --url gdc=${URL} ../gdc/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gdc-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gdc-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/gdc-build.assert
```
...
[gdc/gitian.sigs](https://github.com/mycointest/gitian.sigs/) repository, or if that's not possible to create a pull
request.
