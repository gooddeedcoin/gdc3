...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Gdc Core gdc-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  gdc-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded gdc transaction") + "\n" +
              "  gdc-tx [options] -create [commands]   " + _("Create hex-encoded gdc transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded gdc transaction
